...
%\begin{equation}\label{E:H_HLD}
%\begin{split}
\begin{multline}\label{PE:q*xq_1}
q^*\kvt q\kvt = [\SUM{0}{}\sqrt{\frac{m_b}{N}}u_{\alpha}\lbt e\kvba\EXP{-i\pmb{\kappa}\cdot\mathbf{r}_0\ab{l}{0}}]\\
\times[\SUM{0}{}\sqrt{\frac{m_{b'}}{N}}u_{\alpha'}\lbtp e^*\kvbap\EXP{i\pmb{\kappa}\cdot\mathbf{r}_0\ab{l'}{0}} ]
\end{multline}
% \end{split}
%\end{equation}


\begin{multline}\label{PE:q*xq_2}
q^*\kvt q\kvt = [\SUMb{0}{}\pmb{e}\kvb\cdot\SUMl{0}{}\sqrt{\frac{m_b}{N}}\pmb{u}\lbt \EXP{-i\pmb{\kappa}\cdot\mathbf{r}_0\ab{l}{0}}]\\
\times[\SUMbp{0}{}\pmb{e}^{*'}\kvbp\cdot\SUMlp{0}{}\sqrt{\frac{m_{b'}}{N}}\pmb{u}^{'}\lbtp \EXP{-i\pmb{\kappa}\cdot\mathbf{r}_0\ab{l'}{0}}]
\end{multline}

The sum over $\alpha$ is implied because of the dot product

Special case for b=1:

\begin{multline}\label{PE:q*xq_3}
q^*\kvt q\kvt = [\pmb{e}\kv\cdot\SUMl{0}{}\sqrt{\frac{m}{N}}\pmb{u}\lt \EXP{-i\pmb{\kappa}\cdot\mathbf{r}_0\ab{l}{0}}]\\
\times[\pmb{e}^{*'}\kv\cdot\SUMlp{0}{}\sqrt{\frac{m}{N}}\pmb{u}^{'}\ltp \EXP{-i\pmb{\kappa}\cdot\mathbf{r}_0\ab{l'}{0}}]
\end{multline}

So, we want to know what these terms look like:

\begin{equation}\label{eig:dyadic}
\pmb{e}\kv \times \pmb{e}^{*'}\kv
\end{equation}

This dyadic product is not exactly all the terms in the sum above, but its at least an estimate.

The dyadic product is diagonal only if the eigenvector looks like one of these:

\begin{multline}\label{eig:prim}
\pmb{e}\kv = (a,0,0)
\pmb{e}\kv = (0,a,0)
\pmb{e}\kv = (0,0,a)
\end{multline}

Where $a$ is possibly complex.  Nothing more can be reduced without making some assumptions on the form of eigenvectors.
